              ...
              <div key={item.label} className="group relative flex-shrink-0">
                ...
                {item.hasDropdown && item.hasMegaMenu && item.categories && (
                  <div className="absolute top-full left-1/2 -translate-x-1/2 pt-3 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
                    <div className="rounded-lg shadow-xl p-6 min-w-[600px] bg-[rgb(11,28,63)]/95 border border-white/20" style={{ backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)' }}>
                      <div className="grid grid-cols-3 gap-8">
                        {item.categories.map((category) => (
                          <div key={category.title}>
                            <h3 className="text-accent/90 font-semibold text-[10px] mb-3 uppercase tracking-widest">
                              {category.title}
                            </h3>
                              ...
                              {category.items.map((subItem) => {
                                ...
                                return (
                                  <a
                                    key={subItem}
                                    href="#"
                                    className="block px-3 py-2 text-sm text-white/80 hover:text-white hover:bg-white/5 rounded-md transition-all duration-150"
                                  >
                                    {subItem}
                                  </a>
                                );
                ...
                {item.hasDropdown && !item.hasMegaMenu && item.items && (
                  <div className="absolute top-full left-0 pt-3 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
                    <div className="rounded-lg shadow-xl py-1 min-w-[200px] bg-[rgb(11,28,63)]/95 border border-white/20" style={{ backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)' }}>
                      {item.items.map((subItem) => {
                        ...
                        return (
                          <a
                            key={subItem}
                            href="#"
                            className="block px-4 py-2.5 text-sm text-white/80 hover:text-white hover:bg-white/5 transition-all duration-150"
                          >
                            {subItem}
                          </a>
                        );
                  ...
                  <div className="ml-2 mt-2 space-y-4">
                    {item.categories.map((category, idx) => (
                      <div key={category.title}>
                        ...
                        <div className="text-accent/90 text-[10px] font-semibold mb-2 uppercase tracking-widest px-2">
                          {category.title}
                        </div>
                        {category.items.map((subItem) => {
                          const isActive = isActiveDropdownItem(item.label, subItem);
                          if (subItem === "Law Firms") {
                            return (
                              <Link
                                key={subItem}
                                to="/law-firm-solutions"
                                className={`block px-4 py-2.5 text-sm rounded-md transition-all ${
                                  isActive
                                    ? 'text-white bg-white/10 font-medium'
                                    : 'text-white/80 hover:text-white hover:bg-white/5'
                                }`}
                          ...
                          return (
                            <a
                              key={subItem}
                              href="#"
                              className="block px-4 py-2.5 text-sm text-white/80 hover:text-white hover:bg-white/5 rounded-md transition-colors"
                            >
                              {subItem}
                            </a>
                          );
